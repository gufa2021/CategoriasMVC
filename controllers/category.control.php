      ...
      switch($_GET["acc"]){
        ...
        case "ins":
          ...
          if(isset($_POST["btnacc"])){
            ...
            if($lastID){
              redirectWithMessage("¡Categoría Ingresada!","index.php?page=category&acc=upd&ctgcod=".$lastID);
            }else{
              //Se obtiene los datos que estaban en el post
              $htmlDatos["ctgcod"] = $_POST["ctgcod"];
        ...
        case "upd":
          if(isset($_POST["btnacc"])){
            ...
            if(actualizarCategoria($_POST)){
              ...
              redirectWithMessage("¡Categoría Actualizada!","index.php?page=category&acc=upd&ctgcod=".$_POST["ctgcod"]);
            }
          }
          if(isset($_GET["ctgcod"])){
            $categoria = obtenerCategoria($_GET["ctgcod"]);
            if($categoria){
              $htmlDatos["categoryTitle"] = "Actualizar ".$categoria["ctgdsc"];
              $htmlDatos["categoryMode"] = "upd";
              $htmlDatos["ctgcod"] = $categoria["ctgcod"];
        ...
        if(isset($_POST["btnacc"])){
          //implementar logica de guardado
          if(borrarCategoria($_POST["ctgcod"])){
            //forzando a que se actualice con los datos de la db
